...
public class SerializableFeaturesListView extends JSONSerializableView implements OnClickListener {
	...
	private List<FeatureView> featureViews = new ArrayList<FeatureView>();
	...
	public void writeContentsToBundle(Bundle b) {
		...
		for (int i = 0; i < featureViews.size(); i++) {
			...
			if (!featureViews.get(i).getFeatureName().isEmpty() && !featureViews.get(i).getActuator().isEmpty()) {
				feature.putString("name", featureViews.get(i).getFeatureName());
				feature.putStringArray("actuators", new String[] { featureViews.get(i).getActuator() });
