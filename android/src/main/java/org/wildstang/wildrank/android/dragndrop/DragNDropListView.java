...
public class DragNDropListView extends ListView implements ListViewButtonListener {
	...
	int mDragHandler = 0;
	int mMoveToTopButton = 0;
	int mMoveToBottomButton = 0;
	...
	public void setDragNDropAdapter(DragNDropAdapter adapter) {
		...
		mMoveToTopButton = adapter.getMoveToTopButton();
		mMoveToBottomButton = adapter.getMoveToButtomButton();
		adapter.setListItemButtonClickListener(this);
		setAdapter(adapter);
	}
	...
	private void startDrag(int childIndex, int y) {
		Log.d("startDrag", "drag started at " + childIndex);
		Log.d("startDrag", "mStartPosition " + mStartPosition);
	...
	@Override
	public void onButtonClick(int buttonID, int position) {
		Log.d("onClick", "click at position " + position);
		if (buttonID == mMoveToTopButton) {
			mStartPosition = position;
			startDrag(position, 0);
			stopDrag(position, 0);
		} else if (buttonID == mMoveToBottomButton) {
			mStartPosition = position;
			startDrag(position, 0);
			stopDrag(position, getAdapter().getCount() - 1);
		}
	}
}
